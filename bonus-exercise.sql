...
-- Print Top Agents and the Countries to where they sold as well as the total amount for that country

SELECT AGENTS.AGENT_NAME, CUSTOMER.CUST_COUNTRY, SUM(ORDERS.ORD_AMOUNT) AS TOTAL_AMOUNT
FROM ORDERS
INNER JOIN CUSTOMER ON ORDERS.AGENT_CODE = CUSTOMER.AGENT_CODE
INNER JOIN AGENTS ON CUSTOMER.AGENT_CODE = AGENTS.AGENT_CODE
GROUP BY AGENTS.AGENT_NAME, CUSTOMER.CUST_COUNTRY  
HAVING SUM(ORDERS.ORD_AMOUNT) > 10000
...
SELECT AGENTS.AGENT_NAME, CUSTOMER.CUST_NAME, CUSTOMER.CUST_COUNTRY, ORDERS.ORD_AMOUNT
FROM ORDERS
INNER JOIN CUSTOMER ON ORDERS.AGENT_CODE = CUSTOMER.AGENT_CODE
INNER JOIN AGENTS ON CUSTOMER.AGENT_CODE = AGENTS.AGENT_CODE
WHERE ORDERS.ORD_AMOUNT > 4000;
